<?php
    ...
    class accionesPersonal extends conexion {
        
        private $TAMANO_PAGINA=10;
        
        public function __construct(){
            parent::__construct(); 
        }
        
        public function saveEmploys($formData){
            
            $response;
            
            if(mysqli_query($this->conection, "INSERT INTO Usuarios() VALUES(null, '".$formData['usuario']."', '".$formData['pass']."', '".$formData['apP']."','".$formData['apM']."','".$formData['nombre']."' , '".$formData['rfc']."', '".$formData['curp']."', '".$formData['sexo']."', '".$formData['tel']."', '".$formData['email']."', '".$formData['domicilio']."', '".$formData['fechaN']."','".$formData['edad']."','".$formData['fechaIE']."', '".$formData['grado']."',  ".$formData['estudio']." ,'".$formData['escuela_estudia']."', ".$formData['beca'].",'".$formData['tipo_beca']."', 1, ".$formData['loginGranted'].", '".$formData['permisos']."', '".$formData['nombramiento']."','".$formData['funcion']."' ,'".$formData['tipo_usuario']."', '".$formData['turno']."', '".$formData['localidad']."', '".$formData['municipio']."', '".$formData['fechaIS']."', ".$formData['otroJob'].",'".$formData['otroJob']."', '".$formData['estadoC']."')")){
					
						
				$usuario = mysqli_query($this->conection, "SELECT MAX(id_usuario) FROM Usuarios");
                ...
                return $response = 'Hubo un error:'.mysqli_error($this->conection);	
			}
            
        }
        public function getAllPersonal($PAGES){
            
            $result = array();
            try{
                
                $employes = mysqli_query($this->conection,"SELECT id_usuario,CONCAT(nombre,apellidoP,apellidoM) as nombre FROM Usuarios LIMIT ".$PAGES.",10");
               
                while($rows = mysqli_fetch_assoc($employes)){
                    $result[] = $rows;                   
                }
        ...
        public function getTotalEmployers(){
            $data = mysqli_query($this->conection,"SELECT MAX(id_usuario) from Usuarios");
            $dataMax = mysqli_fetch_row($data);
            $total_paginas = ceil($dataMax / $this->TAMANO_PAGINA);
